...
for i in dados_IPC:
    ...
    # CORREÇÃO: Removido o "* 1000" daqui, pois o limite Y é 0.5 a 1.0
    ax.plot(categorias, dados_IPC[i]['fft'], label='fft', color='#9BBB59', linewidth=2.5,marker='o', markersize=6)
    ax.plot(categorias, dados_IPC[i]['astar'], label='astar', color='#C0504D', linewidth=2.5,marker='s', markersize=6)
    ax.plot(categorias, dados_IPC[i]['kmeans'], label='kmeans', color='#4F81BD', linewidth=2.5,marker='x', markersize=6)
